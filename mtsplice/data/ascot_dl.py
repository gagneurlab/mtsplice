...
class ExonInterval(Interval):
    ...
    def sequence(self):
        ...
        if exon_length > self.length:
            ...
            crop_length = exon_length - self.length
            ...
            if self.strand == "+":
                ...
                seq = seq_r + seq_l
        elif exon_intron_length > self.length:
            crop_length = exon_intron_length - self.length
            ...
            pad_length = self.length - exon_intron_length
...
def trimN(seq):
    ''' Trim "N" from the begining and end
    '''
    import re
    return re.sub(r"^N*|N*$", "", seq)


def anno_region(flanking,
                exonL,
                maxL,
                align='left'):
    ''' Returns onehot annotation information
        Args:
            flanking: length of flanking intron
            exonL: exon length
            maxL: max return sequence length. 
                If orginal sequence smaller, then pad 0
    '''
    exon = np.concatenate([np.zeros(flanking),
                           np.ones(exonL),
                           np.zeros(flanking)])
    intron_up = np.concatenate([np.ones(flanking),
                                np.zeros(exonL + flanking)])
    intron_dw = np.concatenate([np.zeros(flanking + exonL),
                                np.ones(flanking)])
    anno = np.stack([exon, intron_up, intron_dw], axis=-1)
    if len(anno) < maxL:
        pad = np.zeros((maxL - exonL - flanking * 2, 3))
        anno = np.concatenate([anno, pad])
    elif len(anno) > maxL:
        # TODO: also implement trimming from left (align right)
        if align == 'start':
            anno = anno[:maxL]
        elif align == 'end':
            anno = anno[-maxL:]
        else:
            assert "Unknown align"
    return anno
...
class Ascot(Dataset):
        ...
        if self.pad_trim_same_l:
            ...
            if self.encode:
                ...
                if self.seq_align == 'both':
                    seql = encodeDNA([seq], maxlen=self.L, seq_align='start')[0]
                    seqr = encodeDNA([seq], maxlen=self.L, seq_align='end')[0]
                    out["inputs"]["seql"] = seql
                    out["inputs"]["seqr"] = seqr
                    if self.flanking_exons:
                        out["inputs"]["exon_up"] = encodeDNA([exon_up])[0]
                        out["inputs"]["exon_dw"] = encodeDNA([exon_dw])[0]
                else:
                    seq = encodeDNA([seq], maxlen=self.L, seq_align=self.seq_align)[0]
                    out["inputs"]["seq"] = seq
            else:
                out["inputs"]["seq"] = seq
            if self.region_anno:
    ...
    def batch_train_iter(self,
        ...
        """
        while True:
            for x in self.batch_iter(batch_size=batch_size,
                                     **kwargs):
                if x["targets"].shape[0] != batch_size:
                    break
                if sample_weight:
                    if self.region_anno:
                        if self.seq_align == 'both':
        ...
        for x in self.batch_iter(**kwargs):
            ...
            if add_mean:
                if self.region_anno:
                    ...
                    if self.seq_align == 'both' and self.encode:
                        if self.flanking_exons:
                            yield ([x["inputs"]["seql"],
                                    x["inputs"]["seqr"],
                                    x["inputs"]["exon_up"],
...
gtex_to_ascot_tissue_mapping = dict(zip(ascot_to_gtex_tissue_mapping.values(), ascot_to_gtex_tissue_mapping.keys()))

tissues_short = ['Retina - Eye', 
             'RPE_Choroid_Sclera - Eye',
